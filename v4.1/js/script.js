...
var rowBoreCountText = '<p>There are <input type="number" placeholder="0" id="rowBoreCount" min="1"> bores.</p>';
...
$(document).on('click', '#feedback', function() {
  ...
  feedback += "%0D%0A%0D%0AApp Version: v4.1.6";
  ...
  for(var i in permits) {
    ...
    if(permits[i] == "bld") {
      ...
      if($('#fireCategories').val() == "fire alarms") {
        ...
        smifFee = (permitFee * .95) * .1;
        stateFee = 4.5;
      } else if($('#fireCategories').val() == "fire sprinkler tenant improvement") {
        if ($('#fireSprinklerHeads').val() > 0 && $('#fireSprinklerHeads').val() <= 6) {
          permitFee = 222;
        } else if ($('#fireSprinklerHeads').val() > 6 && $('#fireSprinklerHeads').val() <= 20) {
          permitFee = 444;
        } else if ($('#fireSprinklerHeads').val() > 20) {
          permitFee = 688.2 + (3.27 * ($('#fireSprinklerHeads').val() - 20));
        }
        esaFee = (permitFee * .95) * .07;
        smifFee = (permitFee * .95) * .1;
        stateFee = 4.5;
      } else if($('#fireCategories').val() == "new water based fire suppression systems") {
        if ($('#fireSuppressionRisers').val() == 1) {
          permitFee = 688.2;
        } else if($('#fireSuppressionRisers').val() == 2) {
          permitFee = 1376.4;
        } else if ($('#fireSuppressionRisers').val() > 2) {
          permitFee = 1376.4 + (($('#fireSuppressionRisers').val() - 2) * 32.73);
        }
        if($('#fireSuppressionHeads').val() > 20) {
          permitFee += 3.27 * ($('#fireSuppressionHeads').val() - 20);
        }
      ...
      if ($('#rowType').val() == "construction on") {
        if ($('#rconSidewalk').is(':checked')) {
          ...
          if (($('#rowSidewalkSize').val() / 9) > 30) {
            permitFee += 2.56 * (($('#rowSidewalkSize').val() / 9) - 30);
          }
        }
        if ($('#rconDriveway').is(':checked')) {
          permitFee += $('#rowAsphaltDrivewayNum').val() * 230.54;
          permitFee += $('#rowConcreteDrivewayNum').val() * 358.61;
        }
        if ($('#rconCurb').is(':checked')) {
          if ($('#rowGutterLength').val() > 50) {
            permitFee += 2.56 * ($('#rowGutterLength').val() - 50);
          }
        }
        if ($('#rconTrenchBore').is(':checked')) {
          if ($('#rowTrenchType').val() == "an open cut trench") {
            if ($('#rowTrenchLength').val() > 100) {
              permitFee += 243.34 + (parseInt($('#rowTrenchLength').val() - 100) * .9);
            } else {
              permitFee += 243.34;
            }
          } else if ($('#rowTrenchType').val() == "a monitoring well, a bore, or potholing") {
            permitFee += $('#rowBoreCount').val() * 76.85;
          }
        }
        if ($('#rowPaving').val() > 3000) {
          permitFee += 230.54 + (Math.round(($('#rowPaving').val() - 3000) / 1000) * 38.42);
        } else {
          permitFee += 230.54;
        }
      } else if ($('#rowType').val() == "use of") {
        if ($('#rowUse').val() == "a special motor vehicle") {
          ...
          if ($('#rowUseDays').val() == 1) {
            ...
            permitFee = 195.16 + (($('#rowUseDays').val() - 31) * 13.01);
          }
        } else if ($('#rowUse').val() == "a banner") {
          if ($('#rowBannerInstall').val() == "city") {
            permitFee += 46.39 + ($('#rowBannerBlocks').val() * 32.53);
          } else if ($('#rowBannerInstall').val() == "private") {
            permitFee += $('#rowBannerBlocks').val() * 78.06;
          }
        }
      } else if ($('#rowType').val() == "utility work in") {
        if ($('#rowTrench').val() == "will") {
          permitFee += 243.34 + (($('#rowTrenchLength').val() - 100) * .9);
        }
        if ($('#rowBores').val() == "will") {
          permitFee += $('#rowBoreCount').val() * 76.85;
        }
      ...
      if($('#pgrow .daysForBarricade').val() > 0 && $('#pgrow .daysForBarricade').val() <= 5) {
        permitFee += parseInt($('#pgrow .daysForBarricade').val()) * 52.04;
